...
class DBHelperPeriod {
  ...
  deleteTable() {
    db.then((value) => value.execute("DROP TABLE $TABLE"));
  }
  ...
  Future<int> update(Period period) async {
    ...
    return await dbClient.update(TABLE, map, where: '$ID = ?', whereArgs: [
      period.id,
    ]);
  }
...
class DBHelperGrades {
  static Database _db;
  static const String ID = 'id';
  static const String WEIGHT = 'weight';
  static const String COURSEID = 'courseId';
  static const String GRADE = 'grade';
  static const String ORDER = 'orderO';
  static const String TITLE = 'title';
  static const String TABLE = 'grades';
  static const String DB_NAME = 'grade1.db';
  ...
  _onCreate(Database db, int version) async {
    await db.execute(
        "CREATE TABLE $TABLE ($ID TEXT PRIMARY KEY, $TITLE TEXT, $WEIGHT REAL, $COURSEID TEXT, $GRADE REAL, $ORDER INTEGER)");
  }
  ...
  Future<Grade> save(Grade grade) async {
    var dbClient = await db;
    await dbClient.insert(TABLE, grade.toMap());
    return grade;
    /*
    await dbClient.transaction((txn) async {
      var query = "INSERT INTO $TABLE ($NAME) VALUES ('" + grade.name + "')";
      return await txn.rawInsert(query);
    });
    */
  }
  ...
  Future<List<Grade>> getGrades() async {
    var dbClient = await db;
    List<Map> maps = await dbClient.query(TABLE,
        columns: [ID, WEIGHT, COURSEID, GRADE, ORDER, TITLE],
        orderBy: "$ORDER");
